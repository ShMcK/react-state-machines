...
export function AutoComplete() {
	...
	function onHover(event: React.MouseEvent) {
		...
		if (
			...
			itemIndex !== current.context.activeIndex
		) {
	...
	return (
		<div>
			...
			{current.matches("Error") && (
				<div>{current.context.errorMessage}</div>
			)}
			{current.matches("Results") && (
				<>
					<div onClick={onSelect} onMouseOver={onHover}>
						{current.context.results.map(
							(item: any, index: number) => (
								<div
									...
									data-index={index}
								>
									{index === current.context.activeIndex
										? "*"
										: " "}{" "}
									{item.name}
								</div>
			...
			{current.matches("Done") && (
				<div>
					!{" "}
					{current.context.results[current.context.activeIndex]?.name}
				</div>
			)}
		</div>
	);
